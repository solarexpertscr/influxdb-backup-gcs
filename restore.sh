...
GITHUB_RAW_URL="https://raw.githubusercontent.com/solarexpertscr/workspace/main/influxdb-backup-gcs/restore.sh"
GITHUB_PAT_FILE="/opt/influxdb-backup-gcs/.github-pat"
GITHUB_PAT=""
...
do_update() {
    ...
    if ! curl -fsSL -H "Authorization: token ${GITHUB_PAT}" "$GITHUB_RAW_URL" -o "$temp_script"; then
